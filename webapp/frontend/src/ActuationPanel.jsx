...
// picker, Start, and the in-session Stop are patient-only. Instead, the
// backend (webapp/backend/sessions.py) looks at the patient's logged session
// history and recommends the next weight to try; the clinician's job on this
// tab is reviewing that history and approving/rejecting the recommendation,
// plus the raw Twist/Untwist jog controls (a supervised fine-adjustment tool,
// not something a patient needs unsupervised access to). Force Stop is the
// one exception left available to BOTH roles - it's a safety escape hatch,
// not a way to run the exercise, so it shouldn't require the patient to be
// the one holding it. The recommendation itself is a nudge, not a lock -
// once approved it just surfaces a "use this weight" button on the patient's
